...
Delaunay::Delaunay(glm::vec3 punto1, glm::vec3 punto2, glm::vec3 punto3, glm::vec3 punto4, float tol)
...
int Delaunay::enQueTriangulo(glm::vec3 &punto) const {
	int i_tri = 0;//rand()%triangulos.size();
	...
	while (i_tri!=-1) {
		...
		if (ff[0] < ff[2]) {
			if (ff[0] < ff[1]){
				imin=0; //Punto 0 es el de menor peso
			} else {
				imin=1; //Punto 1 es el de menor peso
			}
		} else {
			if (ff[2] < ff[1]) {
				imin=2; //Punto 2 es el de menor peso
			} else {
				imin=1; //Punto 1 es el de menor peso
			}
		}
		
		//Si el peso minimo no es negativo ya lo encontró
		if (ff[imin]>=0) {
			break;
		}
		
		//Sino busca en el tringulo vecino hacia donde el peso minimo se hace negativo
		//esto da la pauta hacia que "direccion" ir buscando
