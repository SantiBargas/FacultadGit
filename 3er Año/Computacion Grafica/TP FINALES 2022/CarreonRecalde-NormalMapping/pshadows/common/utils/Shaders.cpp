...
void Shader::setBuffers (const GeometryRenderer & geo) {
	...
	// Aca tengo que hacer los mismos para tangente y bitangente
	
	GLint loc_tangents = glGetAttribLocation(program_id, "vertexTangents"); 
	if (loc_tangents!=-1) { // texture coords
		glBindBuffer(GL_ARRAY_BUFFER,geo.tangentsVBO());
		cg_assert(geo.tangentsVBO()!=0,"Geometry does not have tangents coordinates");
		glVertexAttribPointer(loc_tangents, 2, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(loc_tangents);
	}
	
	GLint loc_bi_tangents = glGetAttribLocation(program_id, "vertexBiTangents"); 
	if (loc_bi_tangents!=-1) { // texture coords
		glBindBuffer(GL_ARRAY_BUFFER,geo.bi_tangentsVBO());
		cg_assert(geo.bi_tangentsVBO()!=0,"Geometry does not have tangents coordinates");
		glVertexAttribPointer(loc_bi_tangents, 2, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(loc_bi_tangents);
	}
