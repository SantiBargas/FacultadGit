...
Shader::Shader (const std::string &vertex_fname, const std::string &fragment_fname, const std::string &geometry_fname) {
	// AGREGADO: si hay geometry shader, cargarlo
	if (geometry_fname.empty()) {
		load(vertex_fname,fragment_fname);
	} else {
		load(vertex_fname,fragment_fname,geometry_fname);
	}
}

Shader::Shader (const std::string &fname, bool with_geo) {
	if (with_geo)
		load(fname+".vert",fname+".frag",fname+".geo");
	else
...
void Shader::load(const std::string &vertex_fname, const std::string &fragment_fname, const std::string &geometry_fname) {
	...
	GLuint geometry_id;
	// AGREGADO: geo shader
	if (!geometry_fname.empty()) geometry_id = loadAndCompile(GL_GEOMETRY_SHADER,geometry_fname);
	...
	// AGREGADO: geo shader
	if (!geometry_fname.empty()) glAttachShader(program_id,geometry_id);
	...
	// AGREGADO: geo shader
	if (!geometry_fname.empty()) glDetachShader(program_id,geometry_id);
	
	glDeleteShader(vertex_id);
	glDeleteShader(fragment_id);
	// AGREGADO: geo shader
	if (!geometry_fname.empty()) glDeleteShader(geometry_id);
}

void Shader::load(const std::string &fname, bool with_geo) {
	if (with_geo)
		load(fname+".vert",fname+".frag",fname+".geo");
	else
		load(fname+".vert",fname+".frag");
...
void Shader::setMatrixes (const glm::mat4 &object, const glm::mat4 &camera, const glm::mat4 &view, const glm::mat4 &projection) {
	setUniform("objectMatrix",object);
	setUniform("cameraMatrix",camera);
