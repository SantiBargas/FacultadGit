...
Model Model::loadSingle(const std::string &name, int flags) {
	ObjMesh mesh = readObj("models/"+name+".obj");
	if (!(flags&fDontFit)) centerAndResize(mesh.positions);
	Geometry geometry = toGeometry(mesh,0);
	if (flags&fRegenerateNormals or geometry.normals.empty()) geometry.generateNormals();
	if (flags&fNoTextures) mesh.parts[0].material.texture.clear();
	return Model(std::move(geometry), mesh.parts[0].material, mesh, flags&fKeepGeometry);
}
...
std::vector<Model> Model::load(const std::string &name, int flags) {
	...
	std::vector<Model> vret; vret.reserve(mesh.parts.size());
	for (auto &part : mesh.parts) {
		Geometry geometry = toGeometry(mesh,part);
		if (flags&fRegenerateNormals or geometry.normals.empty()) geometry.generateNormals();
		if (flags&fNoTextures) part.material.texture.clear();
		vret.emplace_back(std::move(geometry), part.material, mesh, flags&fKeepGeometry);
	}
	return vret;
}
