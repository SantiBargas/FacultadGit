...
void outlineInstance(const Instance &instance, glm::vec3 color_silhouette) {
	...
	//DIBUJAMOS LA GALLINA CHIQUITA EN EL STENCIL
	glEnable(GL_STENCIL_TEST); //Activamos el stencil
	glStencilOp(GL_REPLACE,GL_KEEP,GL_KEEP); //Si falla el stencil test reemplazamos,si falla el depth test se mantiene, si pasa el depth test se mantiene
	glStencilFunc(GL_NEVER,1,0xFF);   //Si falla el stencil dibujamos con valor 1
	drawInstance(instance,shader_texture); //Dibujamos la gallina en el stencil
	
	//DIBUJAMOS SOLO LA SILUETA DE LA GALLINA QUE NO ESTA TAPADA
	glStencilOp(GL_KEEP,GL_REPLACE,GL_INCR); //Incrementamos para que cada vez que dibuje se incremente 1 en el stencil y no vuelva a solapar la transparencia
	glStencilFunc(GL_EQUAL,0,0xFF);    //Compara donde sea 0
	drawInstance(instance,shader_silhouette); //Dibuja la silueta donde solamente es 0
	
	glDepthFunc(GL_GREATER);
	shader_silhouette.setUniform("color",glm::vec4{color_silhouette,0.5f});
...
//a que gallina le hago click? find_selection
