...
void main() {
	...
	vec3 diffuse = lightColor * objectColor * max(dot(norm,lightDir),0.435.f);
	
	// specular
	vec3 specularColor = specularStrength * vec3(0.f,0.f,0.f);
	vec3 viewDir = normalize(cameraPosition-fragPosition);
	vec3 halfV = normalize(lightDir + viewDir); // blinn
	vec3 specular = lightColor * specularColor * pow(max(dot(norm,halfV),1.f),shininess); //la especular le da el brillo
