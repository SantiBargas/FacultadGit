...
void updateControlPointsAround(Spline &spline, int ctrl_pt) {
	...
	///unos son puntos de control y los otros los puntos que marca el usuario
	glm::vec3 l_i_next = spline.getControlPoint(ctrl_pt + 3); ///obtenemos el siguiente  punto de control
	glm::vec3 l_i_prev = spline.getControlPoint(ctrl_pt - 3); ///obtenemos el anterior punto de control
	glm::vec3 l_i = (l_i_next - l_i_prev)*0.5f ;     ///dividimos por 2 el punto siguiente y el anterior //slide39 

	
	glm::vec3 p1 = spline.getControlPoint (ctrl_pt) - (l_i/(3.f));
	glm::vec3 p2 = spline.getControlPoint (ctrl_pt) + (l_i/(3.f));
	spline.setControlPoint(ctrl_pt - 1 , p1);
	spline.setControlPoint(ctrl_pt + 1 , p2);
	
	
	///1)el metodo elegido Catmull-Rom presenta overshotting
	///no es una caracteristica pq no es buscado, es un bug que pasa por como esta definido el catmullRom
	///donde el pez llega con cierta velocidad y no le da para poder realizar el giro adecuado por eso se pasa
	///y vuelve
	
	///aplicar esa interpolación suave alrededor del punto de control especificado,
	///ajustando los puntos de control vecinos para lograr
	///una curva continua y suave en la spline.
	
	
	///2)
	///mientras mas separado este el punto anterior y posterior mas velocidad habra en ese tramo
	///Para corregir estas variaciones de velocidad y lograr una interpolación más uniforme, 
	///se puede considerar un enfoque como la parametrización de la curva. Esto implica asignar un parámetro 
	///que avanza de manera uniforme a lo largo de la curva,
	///independientemente de la distancia entre los puntos de control

	///3)
	///el hecho de que tenga la misma direccion hace que se vea suave pero el hecho de 
	///que no tenga la misma magnitud hace que de golpe cambie la velocidad. Para que la velocidad sea igual necesito que
	///las derivadas sean exactamente igual (continuidad parametrica)tiene que dar lo mismo de los dos lados
	
	///si quiero solamente que sea suave sin importar la velocidad, no importa si tienen el mismo modulo solo importa que 
	///las derivadas tengan la misma direccion
...
glm::mat4 getTransform(const Spline &spline, double t) {
	...
	glm::vec3 deriv;   					///definimos un vector para la derivada
	
	
	glm::vec3 pos=spline.at(t,deriv);  ///definimos la posicion del pez en la spline con el parametro t pasado en la funcion y la derivada
	...
	m[3] = glm::vec4(pos,1.f); 						///ponemos la posicion de origen en la spline para el pez
