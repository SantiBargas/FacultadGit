...
glm::vec3 warpPoint(const Delaunay &delaunay0, const Delaunay &delaunay1, glm::vec3 p) {
	...
	//delaunay0 es una clase que tiene un vector de triangulos y un vector de puntos
	
	int i_triang = delaunay0.enQueTriangulo(p); //obtengo el indice de que triangulo esta el punto p
	Triangulo triangulo_0 = delaunay0.getTriangulos()[i_triang];  //Buscamos en el vector de triangulos
													             //y Obtengo el triangulo del indice p de la maya sin modificar
	
	//Obtenemos los 3 vertices que arman el triangulo en el cual encontramos el punto p
	glm::vec3 x_0 = delaunay0.getPuntos()[triangulo_0[0]]; //indice del vertice 0
	glm::vec3 x_1 = delaunay0.getPuntos()[triangulo_0[1]]; ////indice del vertice 1
	glm::vec3 x_2 = delaunay0.getPuntos()[triangulo_0[2]]; //indice del vertice 2
						
	
	Pesos pesos_p = calcularPesos(x_0,x_1,x_2,p);  //Calculamos los pesos de los vertices encontrados del triangulo
									//no hace falta calcular nuevos pesos ya que van a ser los mismos pesos por mas que modifiquemos el triangulo_0
	
	// NO AGARRAMOS EL NUEVO TRIANGULO PORQUE LOS INDICES DE LOS VERTICES VAN A SER LOS MISMOS EN DELAUNAY0 O DELAUNAY1
	//Obtenemos las nuevas coordenadas de los puntos en delaunay1 
	glm::vec3 x_0_ = delaunay1.getPuntos()[triangulo_0[0]];
	glm::vec3 x_1_ = delaunay1.getPuntos()[triangulo_0[1]];
	glm::vec3 x_2_ = delaunay1.getPuntos()[triangulo_0[2]];

	float pz = p.z; //Guardamos la z del punto que viene en la funcion para no modificarlo
	
	//Ahora encontramos la posicion del nuevo punto p, usando los pesos del triangulo ya conocido
	glm::vec3 p_1 = x_0_*pesos_p[0] + x_1_*pesos_p[1] + x_2_*pesos_p[2];
		
	//Formamos el nuevo punto p_1 con los pesos del delaunay 0
	//El mismo punto que esta en delaunay0 lo encontramos en delaunay1 
	
	//Asignamosla componente z original a la nueva componente
	p_1.z =pz;
