...
int Delaunay::enQueTriangulo(glm::vec3 &punto) const {
	...
	int i_tri = rand()%triangulos.size();  //selecciona un triagnulo al azar
	
	
	//BUSCA EL VERTICE DE MENOR PESO
	while (i_tri!=-1) {  //mientras no encuentre el triangulo sigue el while
		Pesos ff = calcularPesos(i_tri,punto); //calcula el peso del punto del triangulo seleccionado
		int imin;   //asigna un imin para saber cual es el minimo
		if (ff[0] < ff[2]) {  
			if (ff[0] < ff[1]) imin=0; //si el peso del vertice 0 es menor a los otros dos pone el indice en 0
			else               imin=1; //si el peso del vertice 1 es el menor guarda 1 en el indice
		} else {
			if (ff[2] < ff[1]) imin=2; //si el peso del vertice 2 es el menor guarda 2 en imin
			else               imin=1; 
		}
		if (ff[imin]>=0) break;  //si es mayor a 0, significa que ya encontro el triangulo y corta
								//sale si el menor de los pesos es positivo para asegurarse que este en el triangulo
								//si selecciona un triangulo y todos son positivos ya esta en el triangulo
		//si imin es 0 significa que esta en la misma recta
		i_tri = triangulos[i_tri].vecinos[imin];  //en el triangulo que estas anda al vecino del peso menor
	}
	return i_tri;  //retorna el triangulo que contiene al punto
}
