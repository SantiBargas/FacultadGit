...
Alumno::Alumno(int dni,std::string nombreyape,std::string nombretutor,std::string direccion, std::string fechanac,int edad,int tel, bool pago, std::string sala,ficha_medica f):
			 m_dni(dni),m_nombreyape(nombreyape),m_nombretutor(nombretutor),m_direccion(direccion),
			 m_fechanac(fechanac),m_edad(edad),m_tel(tel),m_pago(pago),m_sala(sala),m_ficha_medica(f){}
...
bool Alumno::VerPago(){return m_pago;}
