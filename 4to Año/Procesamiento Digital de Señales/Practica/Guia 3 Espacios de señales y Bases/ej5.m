...
for p=1:size(nmuestranotas,1)
  ...
  for i=2:size(numyfreq,1) %para cada valor de frecuencia f=1,f=2,f=3....
    freq = numyfreq(i,1); %697 770 852 941
  ...
  for i=2:size(numyfreq,2) %para cada valor de frecuencia
    freq = numyfreq(1,i); %1209 1336 1477

    for j=1:length(fases) %para cada fase
      fase = fases(j);
      senoidal = sin(2*pi*freq*tpulso+fase);
      similitud = dot(xpulso,senoidal);  %entre
