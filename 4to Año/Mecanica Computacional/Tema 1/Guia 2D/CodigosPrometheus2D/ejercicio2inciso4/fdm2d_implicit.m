function [PHI, Q,cantIteraciones] = fdm2d_implicit(K,F,xnode,neighb,model,dt)
...
% ----------------------------------------------------------------------
    ...
    for i=1:nt
      cantIteraciones=cantIteraciones+1
        ...
        Q = [Q Qa];
        if (err < tol)
          disp('Ha completado el calculo IMPLICITO. CORTA POR err<tol');
          printf("\n Cantidad de iteracion: ");
          i
            return;
        endif
    endfor
    disp('Llego la maxima iteracion IMPLICITO corta por iteracion.');
endfunction
