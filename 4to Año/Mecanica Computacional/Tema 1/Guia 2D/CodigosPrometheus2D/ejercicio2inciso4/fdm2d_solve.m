function [PHI,Q,dt,cantIteraciones] = fdm2d_solve(K,F,xnode,neighb,model)
    ...
    if model.ts == 0 % Explícito
        ...
        [PHI,Q,cantIteraciones] = fdm2d_explicit(K,F,xnode,neighb,model,dt);
    elseif model.ts == 1 % Implícito
        disp('Iniciando esquemas temporales...');
        % Paso temporal arbitrario
        dt = model.dt;
        [PHI,Q,cantIteraciones] = fdm2d_implicit(K,F,xnode,neighb,model,dt);
